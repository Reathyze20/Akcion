...
const getRatingStyle = (rating: GomesRating) => {
  switch (rating) {
    ...
    case 'HOLD':
      return { bg: 'bg-yellow-900/20', border: 'border-yellow-600', text: 'text-yellow-500', icon: '⏸️' };
    case 'HIGH_RISK':
...
const getLifecycleStyle = (phase?: LifecyclePhase) => {
  switch (phase) {
    ...
    case 'GOLD_MINE':
      return { bg: 'bg-amber-900/30', text: 'text-amber-400', icon: '⛏️' };
    case 'WAIT_TIME':
  ...
  return (
    <div className={`rounded-lg border-2 ${ratingStyle.border} ${ratingStyle.bg} p-6 space-y-4`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <ScoreItem label="Story" value={score.story_score} max={2} />
        <ScoreItem label="Breakout" value={score.breakout_score} max={2} />
        <ScoreItem label="Insider" value={score.insider_score} max={2} />
        <ScoreItem label="ML Pred" value={score.ml_score} max={2} />
        <ScoreItem label="Volume" value={score.volume_score} max={1} />
        {score.earnings_penalty < 0 && (
          <ScoreItem label="Earnings ⚠️" value={score.earnings_penalty} max={0} negative />
        )}
      </div>
      ...
      {(score.green_line || score.red_line || score.is_undervalued) && (
        <div className="bg-black/30 rounded-lg p-4 space-y-2">
          <div className="text-xs text-gray-400 font-semibold mb-2">PRICE TARGETS</div>
          <div className="grid grid-cols-2 gap-4">
            {(score.green_line || score.is_undervalued) && (
              <div>
                <div className="text-xs text-green-400 mb-1">Green Line (BUY)</div>
                <div className="text-lg font-bold text-green-300">
                  {score.green_line ? `$${score.green_line.toFixed(2)}` : 'Undervalued'}
                </div>
              </div>
            )}
            {score.red_line && (
              <div>
                <div className="text-xs text-red-400 mb-1">Red Line (SELL)</div>
      ...
      {score.firing_on_10_cylinders !== undefined && score.firing_on_10_cylinders !== null && (
        <div className={`rounded-lg p-3 border ${
          ...
          <div className="flex items-center gap-2">
            <span className="text-xl">{score.firing_on_10_cylinders ? '✅' : '⚠️'}</span>
            <div>
              <div className="text-xs text-gray-400">Execution Quality</div>
              <div className={score.firing_on_10_cylinders ? 'text-green-300' : 'text-orange-300'}>
                {score.firing_on_10_cylinders ? 'Firing on all 10 cylinders' : 'NOT at 10 cylinders'}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-black/30 rounded-lg p-4">
        <div className="text-xs text-gray-400 font-semibold mb-2">ANALYSIS</div>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {score.bull_case && (
            <div className="bg-green-900/10 border border-green-500/20 rounded-lg p-3">
              <div className="text-xs text-green-400 font-semibold mb-2">🐂 BULL CASE</div>
          ...
          {score.bear_case && (
            <div className="bg-red-900/10 border border-red-500/20 rounded-lg p-3">
              <div className="text-xs text-red-400 font-semibold mb-2">🐻 BEAR CASE</div>
      ...
      {score.catalysts && score.catalysts.length > 0 && (
        <div className="bg-black/30 rounded-lg p-4">
          <div className="text-xs text-green-400 font-semibold mb-2">✨ CATALYSTS</div>
      ...
      {score.risk_factors.length > 0 && (
        <div className="bg-red-900/10 border border-red-500/20 rounded-lg p-4">
          <div className="text-xs text-red-400 font-semibold mb-2">⚠️ RISK FACTORS</div>
          <ul className="space-y-1">
      ...
      <div className="flex items-center gap-4 text-xs text-gray-400 border-t border-white/10 pt-3">
        <span>Data: {score.has_transcript && '📝'} {score.has_swot && '📊'} {score.has_ml_prediction && '🤖'}</span>
        {score.earnings_date && (
          <span>Earnings: {new Date(score.earnings_date).toLocaleDateString()}</span>
        )}
