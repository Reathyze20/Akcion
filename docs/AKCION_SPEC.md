...
11. Test suites: **154 passed**; the 28 failures in old `test_api_*`/`test_yahoo_cache`/`test_phase1_extraction` files are pre-existing (identical set on the base commit) and test obsolete API shapes.
...
1. **Reconcile SMSI position after 1:5 reverse split** (user decision — position edit or broker CSV re-import; a false TRIM fires until then). Consider corporate-action detection as a follow-up guard.
